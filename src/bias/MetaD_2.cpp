    ...
    */
    ...
    private:
      struct Gaussian
      {
        ...
        Gaussian(const vector<double> &center, const vector<double> &sigma, double height, bool multivariate) : center(center), sigma(sigma), height(height), multivariate(multivariate), invsigma(sigma)
        {
      ...
      struct TemperingSpecs
      {
        ...
        double threshold;
        double alpha;
        inline TemperingSpecs(bool is_active, const std::string &name_stem, const std::string &name, double biasf, double threshold, double alpha) : is_active(is_active), name_stem(name_stem), name(name), biasf(biasf), threshold(threshold), alpha(alpha)
        {
        }
      };
                                                ...
                                                last_step_warn_grid(0)
    {
      ...
      if (welltemp_ || dampfactor_ > 0.0 || tt_specs_.is_active)
      {
        ...
        if (welltemp_)
          greatest_alpha = max(greatest_alpha, 1.0);
        if (dampfactor_ > 0.0)
          greatest_alpha = max(greatest_alpha, 1.0);
        if (tt_specs_.is_active)
          greatest_alpha = max(greatest_alpha, tt_specs_.alpha);
        // Find the least alpha.
        double least_alpha = 1.0;
        if (welltemp_)
          least_alpha = min(least_alpha, 1.0);
        if (dampfactor_ > 0.0)
          least_alpha = min(least_alpha, 1.0);
        if (tt_specs_.is_active)
          least_alpha = min(least_alpha, tt_specs_.alpha);
        ...
        for (unsigned i = 0; i < getNumberOfArguments(); i++)
        {
          ...
          else
          {
            if (mesh > 0.5 * sigma0min_[i] || sigma0min_[i] < 0.)
              log << "  WARNING: to use a METAD with a GRID and ADAPTIVE you need to set a Grid Spacing lower than half of the Gaussians \n";
          }
        }
      ...
      if (rect_biasf_.size() > 0 && walkers_mpi)
        log << plumed.cite(
    ...
    void MetaD_2::update()
    {
      ...
      bool nowAddAHill;
      if (getStep() % current_stride == 0 && !isFirstStep)
        nowAddAHill = true;
      else
      {
        nowAddAHill = false;
